...
## Merchant onboarding (Parakeet/Baba/Suraj), wallet-scraper partial-read bug, and Drive API key rotation, 2026-08-11

Follow-on session, same day as the entries below. Three threads:

**1. Onboarded 3 new merchants**: PARAKEET ENGINEERING PVT LTD (`MER_BE152E9A611E`), BABA ENTERPRISES (`MER_AACC5365BC9F`), SURAJ WELLNESS PRIVATE LIMITED (`MER_DBC71D6C79ED`). Added to `data/paynix-merchant-logins.json` (mirrored to the `PAYNIX_MERCHANT_LOGINS` secret) and `data/paynix-commission-rates.json` (rates from the live Google Sheet). Parakeet and Suraj Wellness are Ansh Part clients — added to `ATMOON_MERCHANT_IDS` in both `telegram-alert.js` and `filter-paynix-atmoon.js`. Baba Enterprises has a **"Rahul part" rate-card column** (0.10% / Rs0 below 1000) with no corresponding calculation logic anywhere in this codebase (no `rahulPct` field, no `calcRahulCommission()`, no dashboard column) — onboarded everywhere else, but this cut is **not tracked yet**, pending a decision on whether it's worth building out (mirrors how `calcAnsCommission()`/Ansh Part was added previously — see "Ansh Part commission added to dashboard" section below for the pattern to follow if this gets picked up).

**2. Wallet-log scraper partial-read bug found and fixed.** VELCYNTRA's ₹3,00,000 top-up request (`LR_22C9D8FCDC74`) silently disappeared from two consecutive CI scrapes despite the live DOM table reading correctly and consistently (5/5 manual checks) moments later — a CI-environment-only partial read (1 of 2 rows) that the existing 0-row empty-read guard in `download-paynix-merchant-wallets.js` doesn't catch, since it only fires on a *total* empty read. Fixed: any previously-seen `requestId` missing from the current scrape is now merged back in (Paynix never deletes load requests, so a disappearance is always scrape loss, not reality). Same class of bug as the empty-read guard added 2026-08-08 for Emervex — worth remembering this pattern (safeguards written for one specific failure shape don't automatically cover a milder version of the same failure) if another variant shows up.

**3. Drive API key got rate-limited/blocked by Google, broke the live dashboard for everyone.** Root cause: heavy automated `curl`/direct-fetch testing against the public `googleapis.com` + API-key `alt=media` endpoint during same-session debugging tripped Google's anti-abuse detection ("Sorry... but your computer or network may be sending automated queries"), returning HTML instead of JSON to every visitor — both `docs/index.html` and `docs/paynix-atmoon.html` use this exact endpoint client-side, and `scripts/lib/drive-fetch.js`'s `fetchPreviousFromDrive()` uses the identical pattern server-side (CI's previous-baseline fallback for diffing). **Fix applied**: rotated to a fresh API key (`AIzaSyBYoYTsDm9E40tEQYRxiBCBiQytidLZ1Pw`), updated in `docs/index.html`, `docs/paynix-atmoon.html`, local `.env`, and the `ENV_FILE` GitHub secret (`gh secret set ENV_FILE --repo rishav414-blip/pixlerpay-commission < .env`) — verified working via a clean CI run (`31497491762`) with the suspended-merchant fetch (same anonymous-key code path) succeeding.

**Longer-term fix, planned but not yet built** (explicit user decision: document now, build later): the root problem is that both public dashboard pages call `googleapis.com` directly from the browser with a bare, unauthenticated API key — any burst of automated traffic against that exact URL (mine today, or anyone else's later, or even a legitimate traffic spike) can retrigger Google's bot-detection block, and it affects every visitor simultaneously since they all share the one key. The fix is to stop calling Google directly from the client and put a thin proxy in between:

- **What to build**: a small serverless function (Cloudflare Worker is the natural fit — free tier, no server to maintain, trivial to deploy) that the dashboard fetches from instead of `googleapis.com` directly. The worker holds the Drive API key (or better, a service-account credential) server-side, never exposed to the browser, and forwards/caches the same file reads. `docs/index.html` and `docs/paynix-atmoon.html` swap their `DRIVE_API_KEY`/`fetch(...)` calls for a call to the worker's URL instead.
- **Why this fixes it structurally, not just today**: Google's bot-detection is IP/traffic-pattern-based against the key; a proxy means only the worker's own IP ever talks to Google, and the worker can add its own caching (e.g. cache each file for 60s) so even a burst of dashboard page-loads collapses into one real Drive request — both removes the exposed key from public HTML source (currently visible to anyone who views source) and drastically cuts real request volume to Google.
- **Scope/cost**: new external dependency (Cloudflare account, or equivalent — Vercel/Netlify Edge Functions would also work), a new small codebase to maintain, DNS/CORS configuration, and a secrets question (worker's own Drive credential, rotated independently of the browser-exposed one being retired). Not a same-day change — deliberately deferred rather than rushed in as a same-session follow-on to today's outage fix.
- **Immediate mitigation already in place instead**: just the key rotation above. If this happens again before the proxy is built, the fix is the same — rotate the key in the 4 places listed in point 3, no code changes needed.
...
## Ravino/Vijaj onboarding, merchant-name fix, and wallet-scraper same-run retry, 2026-08-13

Onboarded 2 new merchants: RAVINO TRADERS PRIVATE LIMITED (`MER_B3DE4FC343D8`, portal login name TheHyperMarshal) and VIJAJ TRADERS PRIVATE LIMITED (`MER_59AE1E1BE1A6`, portal login name GuriFashion) — both AK-commission clients (0.10%, no Ansh Part), rates from the live Google Sheet. Both route wallet top-up alerts to a new shared "Paynix: Ravino and Vijaj" Telegram group (chat ID captured via the bot's `getUpdates` after the user added it and sent one message — same technique as the Atmoon/Baba groups earlier).

**Merchant-name fix**: `data/paynix-merchant-logins.json`'s `merchantName` field for these two was the portal *login* name (GuriFashion/TheHyperMarshal), not the reseller-portal legal name — this field feeds every alert/log message, so Telegram showed the wrong name. Fixed at the source (renamed to `VIJAJ TRADERS PRIVATE LIMITED`/`RAVINO TRADERS PRIVATE LIMITED`), re-mirrored to the `PAYNIX_MERCHANT_LOGINS` secret. Confirmed live in a subsequent CI run's log.

**Wallet-scraper same-run retry added** (`download-paynix-merchant-wallets.js`): while verifying the above, found VIJAJ TRADERS' ₹20,50,000 top-up request (`LR_FE51A1958CC0`) had been silently missed — a genuine 0-row DOM read (table not finished rendering) hit 3 times in one day for this merchant specifically, each time confirmed for real via a direct Paynix API check (the entry genuinely existed, wasn't a scrape ghost). The existing run()-level fallback correctly preserved the old baseline rather than corrupting it to empty, but that only means the entry would eventually surface on some *later* run, not this one — so it never triggered an alert when it actually happened. Added a same-run retry (wait 2s, re-check once) before trusting a 0-row read, mirroring the pattern `download-paynix.js`'s failed-payout scraper already had. Tested live immediately after: VIJAJ TRADERS read correctly on the first pass, no more 0-row miss.

**This narrows the failure window, it doesn't eliminate it** — a render delay longer than ~3s total (initial wait + retry wait) could still slip through once in a while. **If this same pattern (a specific merchant's wallet top-up missing/delayed, especially a large one) shows up again, the next escalation is to stop reading the DOM table for wallet-log entries entirely and switch to the authenticated JSON API instead** (`GET /merchant/portal/wallet/load-requests?page=1&per_page=N` — same technique already used for payouts in `download-paynix-merchant-reports.js`, and already manually verified working during this session's investigation). That would remove DOM-render timing as a factor completely, at the cost of a small refactor (the response shape differs slightly from the scraped table's columns — status casing, timestamp format — would need re-mapping to match what `telegram-alert.js` and the dashboard expect). Deliberately not done proactively this session since the retry fix hadn't even been tried yet — only worth the extra work if the retry fix turns out insufficient.
